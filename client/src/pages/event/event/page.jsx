...
function Event() {
    ...
    const timerText = start && tick < start
        ? `До начала: ${fmt(start - tick)}`
        : start && end && tick <= end
            ? `До конца: ${fmt(end - tick)}`
            : end && tick > end
                ? `После завершения: ${fmt(tick - end)}`
                : "Таймер недоступен"
    ...
    return (
        <div className="user-event">
            ...
            <div className="user-event--invite">
                <button className="user-event--invite--qr" onClick={() => setQrOpen(true)}>
                    <img alt="QR приглашения" width="72" height="72" src={qrSrc} />
                </button>
            ...
            {!guestKey && (
                <button className="button" onClick={handleJoin} disabled={joinLoading}>
                    {joinLoading ? "Входим..." : "Войти в мероприятие"}
                </button>
            )}

            <Link
                to={`/event/${event.code}/camera`}
                className="button"
                style={{ opacity: canTakePhoto ? 1 : 0.5, pointerEvents: canTakePhoto ? "auto" : "none" }}
            >
                {status === "Запланировано"
                    ? "Фото доступны после старта"
                    : status === "Завершено"
                        ? "Мероприятие завершено"
                        : !guestKey
                            ? "Сначала войдите в мероприятие"
                            : "+ Сделать фото"}
            </Link>
            ...
            <div className="user-event--gallery">
                ...
                {revealAllowed && gallery.length > 0 && (
                    <div className="user-event--gallery--grid">
                        {gallery.map((photo, idx) => (
                            <div key={`${photo.file_path}_${idx}`} className="user-event--gallery--item">
                                <img src={`${apiRoot}${photo.url}`} alt="event" />
                            </div>
                        ))}
                    </div>
                )}
            ...
            {qrOpen && (
                <div className="system-modal-backdrop" onClick={() => setQrOpen(false)}>
                    <div className="system-modal" onClick={(e) => e.stopPropagation()}>
                        <h4>QR приглашения</h4>
                        <img src={qrSrc} alt="qr-full" style={{ width: "100%", borderRadius: 8 }} />
                        <button className="button" onClick={() => setQrOpen(false)}>Закрыть</button>
                    </div>
                </div>
            )}
        </div>
