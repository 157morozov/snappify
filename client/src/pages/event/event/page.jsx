...
function Event() {
    ...
    if (status === 'Завершено' && !showConfetti) setTimeout(() => setShowConfetti(true), 0)
    ...
    return <div className="user-event">
        {showConfetti && status === 'Завершено' && <div className='confetti'>🎉 ✨ 🎊</div>}
        ...
        <div className="user-event--gallery">
            ...
            {revealAllowed && gallery.length > 0 && <div className="user-event--gallery--grid">{gallery.map((photo, idx) => <div key={`${photo.file_path}_${idx}`} className="user-event--gallery--item"><img src={`${import.meta.env.VITE_API_BASE?.replace('/api','') || 'http://localhost:8000'}${photo.url}`} alt="event" /></div>)}</div>}
        </div>
